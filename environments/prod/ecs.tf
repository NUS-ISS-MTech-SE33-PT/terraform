...
module "ecs" {
  ...
  services = {
    review_service = {
      ...
      security_group_ids = [aws_security_group.ecs_service_sg["review_service"].id]
      environment = [
    ...
    spot_service = {
      ...
      security_group_ids = [aws_security_group.ecs_service_sg["spot_service"].id]
      environment = [
        { name = "HTTP_PORTS", value = "8080" },
      ]
    }
    spot_submission_service = {
      ...
      security_group_ids = [aws_security_group.ecs_service_sg["spot_submission_service"].id]
      environment = [
        { name = "HTTP_PORTS", value = "8080" },
        { name = "SpotSubmissionStorage__BucketName", value = module.s3_spot_submission_photos.bucket },
        { name = "SpotSubmissionStorage__KeyPrefix", value = "submissions/" },
